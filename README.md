...
- **预期价 / 上限价**：在输入框填目标价（停顿 0.4 秒自动保存）；最新价跌破预期价或涨破上限价时，整行绿色高亮。
...
| `POST` | `/api/watch/<symbol>/expect` | 保存预期价/上限价 `{expect?, upper?}`（旧字段 `{price}` 仍兼容，作为预期价）|
...
对「今日关注」板块中设置了目标价（**预期价** 或 **上限价**）的股票，当最新价**下穿预期价**或**上穿上限价**时，自动推送一条微信通知；另支持**单日急跌**提醒。
...
2. **在页面填目标价**：打开面板 → 「今日关注」板块 → 点行首 ⭐ → 在「预期价」/「上限价」输入框填目标价（失焦或停顿自动保存）。
...
- **下穿预期价**：价格从预期价上方下穿到预期价（≤）的瞬间提醒一次；之后持续低于不重复。
- **上穿上限价**：价格从上限价下方上穿到上限价（≥）的瞬间提醒一次；之后持续高于不重复。
- **滞回去抖**：提醒后进入未武装状态，必须回升超过 `预期价×(1+滞回%)`（或回落到 `上限价×(1-滞回%)`）才重新武装，杜绝目标价附近来回震荡导致的重复轰炸。默认滞回 1%。
- **启动补发**：启动时若某标的观测价已低于预期价且从未提醒过，补发一次「已触达」通知（防漏报）；随后不再重复。
- **单日急跌**：当日跌幅超过 `ALERT_DAILY_DROP_PCT` 时提醒一次，每标每个自然日一次（默认关闭）。
- **休市不打扰**：默认仅在活跃交易时段（盘前/盘中/盘后）检查，休市期跳过（`ALERT_SESSION_ONLY=0` 可关闭）。
- 修改目标价会重置追踪；预期价与上限价可同时设置、互不干扰。
- 页面「今日关注」行在最新价触达预期价/上限价时会高亮提示。
